    ...
    const fallbackURL = "https://bikinbaru96.blogspot.com/2024/06/blog-post_13.html";

    let shakaIframeReady = false;
    let pendingShakaChannel = null;
    let jwIframeReady = false;
    let pendingJWKey = null;
...
function setupChannels() {
    ...
    function loadEventVideo(container, specificUrl = null, resetActiveId = true) {
        ...
        const url = specificUrl || storedUrl || originalDataUrl;
        const now = new Date();
        const isChannelContainer = container.classList.contains('channel-container');
        ...
        if (!isChannelContainer && isNaN(eventStartTime.getTime())) return;
        ...
        const isJWKey = url.includes('key=');
        const isChannelParam = url.includes('channel=');
        const channelValue = isChannelParam ? new URLSearchParams(url).get('channel') : null;
        const keyValue = isJWKey ? new URLSearchParams(url).get('key') : null;

        function playShaka(channelKey) {
            pendingShakaChannel = channelKey;
            ...
            console.log('[Shaka] Channel Loaded:', channelKey);
        }
        ...
        function playJW(key) {
            ...
            console.log('[JW] Key scheduled:', cleanKey);
        }

        if (isChannelContainer) {
            placeholder.style.display = 'none';
            countdown.style.display = 'none';

            if (channelValue) {
                playShaka(channelValue);
            } else if (keyValue) {
                playJW(keyValue);
            }
        ...
        if (now >= eventStartTime) {
            ...
            setActiveHoverEffect(id);
            console.log('[Countdown] Waiting for event:', id);
        }
    ...
    window.addEventListener('message', (event) => {
        ...
        if (event.data === 'JW_READY') {
            console.log('[JWPlayer] Player READY diterima dari iframe');
            jwIframeReady = true;

            if (pendingJWKey) {
                const iframe = document.getElementById('video-iframe');
                iframe.contentWindow.postMessage({ type: 'SET_VIDEO', key: pendingJWKey }, '*');
                console.log('[JWPlayer] Key dikirim ulang (deferred):', pendingJWKey);
                pendingJWKey = null;
            }
        }
    });
