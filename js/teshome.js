    ...
    async function loadWithETagCache(url, cacheKey) {
        ...
        // ⬇️ Tangani 304 lebih dulu
        if (res.status === 304 && cachedData) {
            console.log(`[ETAG] ${cacheKey}.json: 304 Not Modified – gunakan cache`);
            return JSON.parse(cachedData);
        }

        // ⬇️ Baru validasi kalau status error
        if (!res.ok) throw new Error(`[${cacheKey}] HTTP error: ${res.status}`);

        if (!contentType.includes("application/json")) {
            throw new Error(`[${cacheKey}] Invalid content-type: ${contentType}`);
        }

        const data = await res.json();
        localStorage.setItem(`cache:${cacheKey}`, JSON.stringify(data));
        localStorage.setItem(etagKey, res.headers.get("etag") || "");
        console.log(`[ETAG] ${cacheKey}.json: fetch ulang – ETag =`, res.headers.get("etag"));
        return data;
    }
    ...
    async function loadEventsFromJSON() {
        const playerBaseUrl = "https://bikinbaru96.blogspot.com/p/testsd.html?key=";

        const [events, playerList] = await Promise.all([
            loadWithETagCache("https://newapp.elutuna.workers.dev/event.json", "event"),
            loadWithETagCache("https://newapp.elutuna.workers.dev/sdplayer.json", "player")
        ]);
        ...
        events.forEach(event => {
            ...
            const servers = rawServers.filter(s => s.label && s.label.trim() !== "");
